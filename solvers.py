...
class Solver(object):
    ...
    def train_epoch(self, epoch):
        ...
        for idx, batch in enumerate(self.train_dataset):
            ...
            # train network
            self.train_model.feed_batch(batch) # set the source image and target image
        ...
        for batch_idx, batch in enumerate(self.test_dataset):
            with torch.no_grad():
                ...
                for idx in range(self.opt.interpolate_len):
                    ...
                    cur_tar_pose = cur_alpha * batch['tar_pose'] + (1 - cur_alpha) * batch['src_pose']
                    # print(batch['src_aus'])
                    # print(cur_tar_aus)
                    test_batch = {'src_img': batch['src_img'], 'tar_aus': cur_tar_aus, 'src_aus':batch['src_aus'], 'tar_img':batch['tar_img'], 'tar_pose':cur_tar_pose}
